"use client"

import React, { useState } from "react";
import Image from "next/image";

const  Modal = ({ isOpen, onClose, title, date, newsPhoto, createData }) => {
  const [linkCopied, setLinkCopied] = useState(false);

  if (!isOpen) return null;

  const copyLink = () => {
    const link = window.location.href;
    navigator.clipboard.writeText(link).then(() => {
      setLinkCopied(true);
      setTimeout(() => {
        setLinkCopied(false);
      }, 2000);
    }).catch(err => {
      console.error("Failed to copy link: ", err);
    });
  };

  const shareToSocialMedia = (platform) => {
    const link = encodeURIComponent(window.location.href);
    let url = "";
    switch (platform) {
      case "telegram":
        url = `https://t.me/share/url?url=${link}`;
        break;
      case "whatsapp":
        url = `https://api.whatsapp.com/send?text=${link}`;
        break;
      case "facebook":
        url = `https://www.facebook.com/sharer/sharer.php?u=${link}`;
        break;
      case "instagram":
        // Instagram does not support direct URL sharing, open Instagram app instead
        url = `https://www.instagram.com/`;
        break;
      default:
        break;
    }
    window.open(url, "_blank");
  };

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 px-2">
      <div className="bg-white rounded-3xl p-4 mdx:p-6 mdl:max-w-[770px] w-full">
        <div className="flex justify-between items-start mb-4">
          <div className="flex flex-row items-center">
            <Image
              src={newsPhoto}
              width={500}
              height={500}
              alt={`News Image`}
              className="w-[200px] h-[200px] object-cover"
            />
            <div className="ml-4">
              <h2 className="text-xl font-semibold mdx:w-[236px] mdx:text-[24px]">{title}</h2>
              <p className="text-gray-300 text-[14px] mdx:text-[20px]">{new Date(createData).toLocaleDateString()}</p>
            </div>
          </div>
          <button onClick={onClose} className="text-gray-600 hover:text-gray-900 w-[24px] h-[24px] mdl:w-[32px] mdl:h-[32px]">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none">
              <path d="M12.0007 10.5872L16.9507 5.63721L18.3647 7.05121L13.4147 12.0012L18.3647 16.9512L16.9497 18.3652L11.9997 13.4152L7.05072 18.3652L5.63672 16.9502L10.5867 12.0002L5.63672 7.05021L7.05072 5.63821L12.0007 10.5872Z" fill="#BABABA" />
            </svg>
          </button>
        </div>

        <h3 className="text-[22px] font-semibold mt-4 mdx:text-[30px]">Share the news</h3>
        <p className="text-gray-500 text-[14px] mdx:text-[18px]">
          Learn more about the latest advances in medical imaging! Share this news with friends and colleagues who may be interested.
        </p>
        <div className="flex mt-4 space-x-2 overflow-x-auto invisible-scrollbar mdx:mt-[35px]">
          <button className={`bg-greentxt text-white rounded-2xl px-[45px] py-4 text-nowrap mdx:px-[75px] ${linkCopied ? 'bg-gray-500' : ''}`} onClick={copyLink}>
            {linkCopied ? "Copied link" : "Copy link"}
          </button>
          <button className="px-4 py-2 border border-1 rounded-xl" onClick={() => shareToSocialMedia('instagram')}>
            <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 22 22" fill="none">
              <g clipPath="url(#clip0_219_10425)">
                <path d="M11 1.98086C13.9391 1.98086 14.2871 1.99375 15.443 2.04531C16.5172 2.09258 17.0973 2.27305 17.484 2.42344C17.9953 2.62109 18.3648 2.86172 18.7473 3.24414C19.134 3.63086 19.3703 3.99609 19.568 4.50742C19.7184 4.89414 19.8988 5.47852 19.9461 6.54844C19.9977 7.70859 20.0105 8.05664 20.0105 10.9914C20.0105 13.9305 19.9977 14.2785 19.9461 15.4344C19.8988 16.5086 19.7184 17.0887 19.568 17.4754C19.3703 17.9867 19.1297 18.3563 18.7473 18.7387C18.3605 19.1254 17.9953 19.3617 17.484 19.5594C17.0973 19.7098 16.5129 19.8902 15.443 19.9375C14.2828 19.9891 13.9348 20.002 11 20.002C8.06094 20.002 7.71289 19.9891 6.55703 19.9375C5.48281 19.8902 4.90273 19.7098 4.51602 19.5594C4.00469 19.3617 3.63516 19.1211 3.25273 18.7387C2.86602 18.352 2.62969 17.9867 2.43203 17.4754C2.28164 17.0887 2.10117 16.5043 2.05391 15.4344C2.00234 14.2742 1.98945 13.9262 1.98945 10.9914C1.98945 8.05234 2.00234 7.7043 2.05391 6.54844C2.10117 5.47422 2.28164 4.89414 2.43203 4.50742C2.62969 3.99609 2.87031 3.62656 3.25273 3.24414C3.63945 2.85742 4.00469 2.62109 4.51602 2.42344C4.90273 2.27305 5.48711 2.09258 6.55703 2.04531C7.71289 1.99375 8.06094 1.98086 11 1.98086ZM11 0C8.01367 0 7.63984 0.0128906 6.4668 0.0644531C5.29805 0.116016 4.49453 0.305078 3.79844 0.575781C3.07227 0.859375 2.45781 1.2332 1.84766 1.84766C1.2332 2.45781 0.859375 3.07227 0.575781 3.79414C0.305078 4.49453 0.116016 5.29375 0.0644531 6.4625C0.0128906 7.63984 0 8.01367 0 11C0 13.9863 0.0128906 14.3602 0.0644531 15.5332C0.116016 16.702 0.305078 17.5055 0.575781 18.2016C0.859375 18.9277 1.2332 19.5422 1.84766 20.1523C2.45781 20.7625 3.07227 21.1406 3.79414 21.4199C4.49453 21.6906 5.29375 21.8797 6.4625 21.9312C7.63555 21.9828 8.00937 21.9957 10.9957 21.9957C13.982 21.9957 14.3559 21.9828 15.5289 21.9312C16.6977 21.8797 17.5012 21.6906 18.1973 21.4199C18.9191 21.1406 19.5336 20.7625 20.1437 20.1523C20.7539 19.5422 21.132 18.9277 21.4113 18.2059C21.682 17.5055 21.8711 16.7063 21.9227 15.5375C21.9742 14.3645 21.9871 13.9906 21.9871 11.0043C21.9871 8.01797 21.9742 7.64414 21.9227 6.47109C21.8711 5.30234 21.682 4.49883 21.4113 3.80273C21.1406 3.07227 20.7668 2.45781 20.1523 1.84766C19.5422 1.2375 18.9277 0.859375 18.2059 0.580078C17.5055 0.309375 16.7062 0.120313 15.5375 0.06875C14.3602 0.0128906 13.9863 0 11 0Z" fill="#088133" />
              <path d="M11.002 5.34961C7.88242 5.34961 5.35156 7.88047 5.35156 11C5.35156 14.1195 7.88242 16.6504 11.002 16.6504C14.1215 16.6504 16.6523 14.1195 16.6523 11C16.6523 7.88047 14.1215 5.34961 11.002 5.34961ZM11.002 14.6652C8.97813 14.6652 7.33672 13.0238 7.33672 11C7.33672 8.97617 8.97813 7.33477 11.002 7.33477C13.0258 7.33477 14.6672 8.97617 14.6672 11C14.6672 13.0238 13.0258 14.6652 11.002 14.6652Z" fill="#088133" />
              <path d="M18.193 5.12627C18.193 5.85674 17.6 6.44541 16.8738 6.44541C16.1434 6.44541 15.5547 5.85244 15.5547 5.12627C15.5547 4.3958 16.1477 3.80713 16.8738 3.80713C17.6 3.80713 18.193 4.4001 18.193 5.12627Z" fill="#088133" />
            </g>
            <defs>
              <clipPath id="clip0_219_10425">
                <rect width="22" height="22" fill="white" />
              </clipPath>
            </defs>
          </svg></button>
          <button className="px-4 py-2 border border-1 rounded-xl" onClick={() => shareToSocialMedia('telegram')}>
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
              <g clipPath="url(#clip0_219_10253)">
                <path fillRule="evenodd" clipRule="evenodd" d="M24 12C24 18.6274 18.6274 24 12 24C5.37258 24 0 18.6274 0 12C0 5.37258 5.37258 0 12 0C18.6274 0 24 5.37258 24 12ZM12.43 8.85893C11.2628 9.3444 8.93014 10.3492 5.43189 11.8733C4.86383 12.0992 4.56626 12.3202 4.53917 12.5363C4.49339 12.9015 4.95071 13.0453 5.57347 13.2411C5.65818 13.2678 5.74595 13.2954 5.83594 13.3246C6.44864 13.5238 7.27283 13.7568 7.70129 13.766C8.08994 13.7744 8.52373 13.6142 9.00264 13.2853C12.2712 11.079 13.9584 9.96381 14.0643 9.93977C14.139 9.92281 14.2426 9.90148 14.3128 9.96385C14.3829 10.0262 14.376 10.1443 14.3686 10.176C14.3233 10.3691 12.5281 12.0381 11.5991 12.9018C11.3095 13.171 11.1041 13.362 11.0621 13.4056C10.968 13.5033 10.8721 13.5958 10.78 13.6846C10.2108 14.2333 9.78391 14.6448 10.8036 15.3168C11.2936 15.6397 11.6858 15.9067 12.077 16.1731C12.5042 16.4641 12.9303 16.7543 13.4816 17.1157C13.6221 17.2077 13.7562 17.3034 13.8869 17.3965C14.3841 17.751 14.8307 18.0694 15.3826 18.0186C15.7032 17.9891 16.0345 17.6876 16.2027 16.7884C16.6002 14.6631 17.3816 10.0585 17.5622 8.16097C17.578 7.99473 17.5581 7.78197 17.5422 7.68857C17.5262 7.59518 17.4928 7.46211 17.3714 7.3636C17.2276 7.24694 17.0056 7.22234 16.9064 7.22408C16.455 7.23203 15.7626 7.47282 12.43 8.85893Z" fill="#088133" />
              </g>
              <defs>
                <clipPath id="clip0_219_10253">
                  <rect width="24" height="24" fill="white" />
                </clipPath>
              </defs>
            </svg>
          </button>
          <button className="px-4 py-2 border border-1 rounded-xl" onClick={() => shareToSocialMedia('whatsapp')}>
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
              <path d="M0 23.9999L1.687 17.837C0.645998 16.033 0.0989998 13.988 0.0999998 11.891C0.103 5.33499 5.43799 0 11.993 0C15.174 0.000999998 18.16 1.24 20.406 3.48799C22.6509 5.73599 23.8869 8.72398 23.8859 11.902C23.8829 18.459 18.548 23.7939 11.993 23.7939C10.003 23.7929 8.04198 23.2939 6.30499 22.346L0 23.9999ZM6.59698 20.193C8.27298 21.188 9.87298 21.784 11.989 21.785C17.437 21.785 21.875 17.351 21.878 11.9C21.88 6.43799 17.463 2.01 11.997 2.008C6.54498 2.008 2.11 6.44199 2.108 11.892C2.107 14.117 2.75899 15.783 3.85399 17.526L2.85499 21.174L6.59698 20.193ZM17.984 14.729C17.91 14.605 17.712 14.531 17.414 14.382C17.117 14.233 15.656 13.514 15.383 13.415C15.111 13.316 14.913 13.266 14.714 13.564C14.516 13.861 13.946 14.531 13.773 14.729C13.6 14.927 13.426 14.952 13.129 14.803C12.832 14.654 11.874 14.341 10.739 13.328C9.85598 12.54 9.25898 11.567 9.08598 11.269C8.91298 10.972 9.06798 10.811 9.21598 10.663C9.34998 10.53 9.51298 10.316 9.66198 10.142C9.81298 9.96998 9.86198 9.84598 9.96198 9.64698C10.061 9.44898 10.012 9.27498 9.93698 9.12598C9.86198 8.97798 9.26798 7.51498 9.02098 6.91998C8.77898 6.34099 8.53398 6.41899 8.35198 6.40999L7.78198 6.39999C7.58398 6.39999 7.26198 6.47399 6.98998 6.77199C6.71798 7.06998 5.94999 7.78798 5.94999 9.25098C5.94999 10.714 7.01498 12.127 7.16298 12.325C7.31198 12.523 9.25798 15.525 12.239 16.812C12.948 17.118 13.502 17.301 13.933 17.438C14.645 17.664 15.293 17.632 15.805 17.556C16.376 17.471 17.563 16.837 17.811 16.143C18.059 15.448 18.059 14.853 17.984 14.729Z" fill="#088133" />
            </svg>
          </button>
          <button className="px-4 py-2 border border-1 rounded-xl" onClick={() => shareToSocialMedia('facebook')}>
            <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 22 22" fill="none">
              <g clipPath="url(#clip0_219_10428)">
                <path d="M11 0C4.92492 0 0 4.92492 0 11C0 16.1586 3.55168 20.4873 8.34284 21.6762V14.3616H6.07464V11H8.34284V9.55152C8.34284 5.80756 10.0373 4.0722 13.713 4.0722C14.41 4.0722 15.6125 4.20904 16.1044 4.34544V7.39244C15.8448 7.36516 15.3938 7.35152 14.8337 7.35152C13.0302 7.35152 12.3332 8.03484 12.3332 9.81112V11H15.9262L15.3089 14.3616H12.3332V21.9195C17.78 21.2617 22.0004 16.6241 22.0004 11C22 4.92492 17.0751 0 11 0Z" fill="#088133" />
              </g>
              <defs>
                <clipPath id="clip0_219_10428">
                  <rect width="22" height="22" fill="white" />
              </clipPath>
            </defs>
          </svg>
          </button>
        </div>
      </div>
    </div>
  );
};

export default Modal;